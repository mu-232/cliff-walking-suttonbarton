...
for episode in range(600):
    ...
    while step_count < max_steps:
        ...
        if next_s in cliff_s or next_s==goal_s:
            # reward_sum += get_immediate_reward(next_s, current_a) #adding reward of terminal state before ending the episode!!
            Q[(current_s, current_a)] = Q[(current_s, current_a)] + step_size * (immediate_r - Q[(current_s, current_a)])
            break
        else:
            pass

        #Get the argmax a Q(next_s,a)
        possible_a = []
        for action in a_list:
            if (next_s[0] + action[0], next_s[1] + action[1]) in all_s:
                possible_a.append(action)
            else:
                pass

        max_a = max(possible_a, key=lambda a: Q[(next_s, a)])

        Q[(current_s, current_a)] = Q[(current_s, current_a)] + step_size * (immediate_r + discount * Q[(next_s, max_a)] - Q[(current_s, current_a)])  # Q learning UPDATE
        current_s = next_s
        step_count += 1

    reward_sum_dictionary[episode] = reward_sum
    # print("reward sum:",episode, reward_sum)
...
plt.plot(episodes, total_rewards, linestyle='-', color='m', label='Reward Sum')
